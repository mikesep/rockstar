...
**Common variables** consist of one of the keywords `a`, `an`, `the`, `my`, `your`, `our`, `his`, `her`, or `their` followed by whitespace and a unique variable name, which must contain only lowercase ASCII letters a-z. The keyword is part of the variable name, so `a boy` is a different variable from `the boy`. Common variables are case-insensitive.
